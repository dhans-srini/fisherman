	...
	public void logout() {
		try {
			...
			FacesContext.getCurrentInstance().getExternalContext().redirect(
					FacesContext.getCurrentInstance().getExternalContext().getRequestContextPath() + "/home.xhtml");
		} catch (IOException e) {
		}
	}
	...
	private void createExcel(OutputStream out) {
		try {
			...
			for (int i = 0; i < forms.size(); i++) {
				...
				String type = Utils.getValueFromPageLabels(frm.getType());
				data.put(i + 2, new Object[] { i + 1, frm.getFormCode(), type, frm.getName(), frm.getFathersName(),
