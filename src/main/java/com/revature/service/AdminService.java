...
public class AdminService {
  ...
  private final String userInsertQuery =
      "insert into users (name,user_name,password,created_by,is_active) " + "values(?,?,?,?,?)";
  private final String userUpdWithAllFieldsQuery =
      "update users u set u.name=?,u.user_name=?,u.password=?,u.updated_by=?,"
          + "u.is_active=? where u.id=?";
  private final String userUpdWithoutPwdQuery =
      "update users u set u.name=?,u.user_name=?,u.updated_by=?," + "u.is_active=? where u.id=?";
  private final String userSelectQuery =
      "select id,name,user_name,password,is_admin,is_active from users u where u.user_name=?";
  private final String userByIdSelectQuery =
      "select id,name,user_name,password,is_admin,is_active from users u where u.id=?";
  private final String userUpdQuery = "update users u set u.last_login_time=? where u.user_name=?";
  private final String allFormQuery =
      "select id,code,type,name,biometric_id, fathers_name, is_married,section,created_on,status "
          + "from form f order by f.created_on desc ";
  private final String allUserQuery =
      "select id,name,user_name,is_admin,is_active from users u where u.id!=?";
  ...
  public User validateUser(String userName, String password) throws BusinessException {
    try {
      ...
      if (!isValidUser) {
        throw new BusinessException("User authentication failed.");
      }
      if (!user.getIsActive()) {
        throw new BusinessException("Your account is deactivated. Please contact admin.");
      }
  ...
  public void createUser(Connection con, User user, User creagedBy)
  ...
  public void updateUser(Connection con, User user, User updatedBy)
      throws SQLException, NoSuchAlgorithmException {
  ...
  public List<User> getAllUsers(User user) throws BusinessException {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        userToAdd.setIsActive(rs.getBoolean("is_active"));
        users.add(userToAdd);
      }
      rs.close();
      ps.close();
